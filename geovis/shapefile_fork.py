...
class Reader:
    ...
    def __dbfHeader(self):
        ...
        #self.numpyrecread = numpy.fromfile
        #self.numpyrecorddtypes = numpy.dtype([('','S%d' % fieldinfo[2]) for fieldinfo in self.fields])
        fmt = ''.join(['%ds' % fieldinfo[2] for fieldinfo in self.fields])
        fmtSize = calcsize(fmt)
        self.recorddtypes = (fmt, fmtSize)
        #f = self.__getFileObj(self.dbf)
        #self.numpyrecords = self.numpyrecread(f, self.numpyrecorddtypes, self.numRecords)
        #self.numpyrecordsgen = (each for each in self.numpyrecords)
    ...
    def __record(self):
        ...
        f = self.__getFileObj(self.dbf)
        #recordContents = next(self.numpyrecordsgen) #self.numpyrecread(f, self.numpyrecorddtypes, 1)[0]
        recFmt = self.recorddtypes
        ...
        for (name, typ, size, deci), value in itertools.izip(self.fields, recordContents):
            #bottleneck in this code, esp with string strip funcs, removing them speeds up x4
